...
import {Form, Input, Modal, Spin, DatePicker, Upload, Button, Icon, notification, Select} from "antd"
import moment from "moment"
import {fileApi} from "../../../constants/api"
import {checkUniqueContractCode, checkConflictedActiveTime} from "../../../validators/supplier/supplierTransportContract"
...
class FormModal extends Component {
	...
	showModelHandler = (e) => {
		...
		if (payload) {
			dispatch({
				type: "supplierTransportContract/view",
				payload: payload
			})
		}
		dispatch({
			type: "selectBankAccounts/selection",
			payload: {
	...
	render() {
		...
		const importProps = {
			...
			data: {
				entityType: "supplierTransportContract",
				key: "supplierTransportContract"
			},
			beforeUpload(file) {
		...
		const {children, title, record, payload, bankAccounts} = this.props
		...
		const bankAccountIds = record.bankAccountIds
		return (
			<span>
						...
						<Spin spinning={viewLoading}>
							...
							<div>
								...
								<Form.Item
									...
									label="银行账号"
								>
									{getFieldDecorator("bankAccountIds", {
										initialValue: bankAccountIds,
										rules: [{
											required: true,
											message: "必填"
										}]
										...
										<Select mode="multiple">
											{bankAccounts.map(item =>
												(<Option key={item.id}>{item.name}/{item.accountNumber}</Option>)
											)}
										</Select>
									)
									}
